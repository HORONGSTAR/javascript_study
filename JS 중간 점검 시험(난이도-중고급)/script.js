...
function result1(arr) {
   ...
   value = Math.round(value / arr.length)
   return value
}
console.log(`3번 문제 정답: ${result1(scores)}점`)
...
function result2(avg) {
   let value
   if (avg > 100 || avg < 0) {
      alert('점수 에러입니다.')
   } else {
      if (avg >= 90) value = 'A'
      else if (avg >= 80) value = 'B'
      else if (avg >= 70) value = 'C'
      else if (avg >= 60) value = 'D'
      else value = 'F'
   }
   return value
}
student.grade = result2(student.avg)
...
console.log(`8번 문제 정답: ${String(student.phone).replaceAll('-', '')}`)
...
const day = new Date(student.reg_date)
const after = new Date(day.setDate(day.getDate() + 20))

console.log(`9번 문제 정답: ${after}`)
...
const employeesSeoul = employees.map((employee) => {
...
console.log(`10번 문제 정답: ${JSON.stringify(employeesSeoul, null, 2)}`)
...
employees.map((employee) => (employee.dept = '개발부'))
