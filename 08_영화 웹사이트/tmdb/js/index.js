...
const getPlatingMovies = async (url) => {
   try {
      const response = await fetch(url, options) // promise 객체를 리턴(서버에서 데이터를 가져올때까지 기다린다)
      // console.log(response) // 기다렸다가 다 가져오면 실행
                    ...
                    <div class="card">
                        <a href="./detail.html?movie_id=${movie.id}">
                            <img src="https://image.tmdb.org/t/p/w500${
                               movie.poster_path
                            }" class="card-img-top poster" alt="${movie.title}" />
                        </a>
                        <div class="card-body">
